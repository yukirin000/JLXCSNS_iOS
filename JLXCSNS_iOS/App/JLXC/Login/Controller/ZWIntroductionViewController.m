...
- (void)viewDidLoad {
    ...
    if (!self.enterButton) {
        ...
        [self.enterButton setTitleColor:[UIColor colorWithHexString:ColorYellow] forState:UIControlStateNormal];
        self.enterButton.layer.borderWidth = 0.5;
        self.enterButton.layer.borderColor = [UIColor colorWithHexString:ColorYellow].CGColor;
    }
...
- (void)reloadPages
{
    ...
    [[self scrollViewPages] enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL *stop) {
        ...
        obj.frame = CGRectOffset(self.view.bounds, x, 0);
...
- (CGRect)frameOfEnterButton
{
    ...
    return CGRectMake(self.view.frame.size.width / 2 - size.width / 2, self.view.frame.size.height - size.height-50, size.width, size.height);
}
